...
// NOAA Mauna Loa Observatory – annual mean CO₂ (ppm)
// Source: gml.noaa.gov/webdata/ccgg/trends/co2/co2_annmean_mlo.txt
const CO2_FALLBACK = [
...
// ABS Survey of Income & Housing (biennial, interpolated between survey years)
// Supplemented with World Bank SI.POV.GINI where available
// Gini coefficient: post-tax, post-transfer disposable income
const GINI_FALLBACK = [
...
// ABS Average Weekly Earnings (Cat. 6302.0) × 52 = annual wage
// ABS Residential Property Price Indexes (Cat. 6416.0) = 8 capital cities weighted median
// Ratio = median dwelling ÷ average full-time annual earnings
const HOUSING_DATA = [
...
// Wealth Gini – Australia
// Primary: Credit Suisse / UBS Global Wealth Report (2010–2023, annual)
// Pre-2010: ABS Survey of Income & Housing wealth module + HILDA Survey estimates
// Includes all net worth (financial + non-financial assets minus liabilities)
// Note: wealth Gini is structurally ~2× income Gini due to asset concentration
...
function DeltaBadge({ first, last, unit, color }) {
  ...
  return (
    <div style={{
      ...
      ▲ +{delta.toFixed(1)}{unit} since 2004
      <span style={{ opacity: 0.6 }}>({pct > 0 ? "+" : ""}{pct}%)</span>
    </div>
  );
}
...
function Panel({ title, subtitle, note, data, dataKey, unit, prefix, color, gradId,
  domain, ticks, yticks, refLine, source, isLive, loading, lastValue, firstValue }) {
    ...
    <div style={{
      ...
      {!loading && firstValue != null && (
        <div style={{ marginBottom: "14px" }}>
          <DeltaBadge first={firstValue} last={lastValue} unit={unit} color={color} />
        </div>
      )}
...
export default function Dashboard() {
  const [co2, setCo2] = useState({ data: CO2_FALLBACK, isLive: false, loading: true });
  const [gini, setGini] = useState({ data: GINI_FALLBACK, isLive: false, loading: true });
  ...
  // Fetch CO₂ from NOAA GML
  useEffect(() => {
    fetch("https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_annmean_mlo.txt")
      .then((r) => {
        if (!r.ok) throw new Error("CORS/network");
        return r.text();
      })
      .then((text) => {
        const rows = text.split("\n")
          ...
          .map((l) => {
            ...
            return { year: parseInt(p[0]), value: parseFloat(p[1]) };
          })
          .filter((d) => !isNaN(d.year) && !isNaN(d.value) && d.year >= 2004);
        if (rows.length > 5) {
          setCo2({ data: rows, isLive: true, loading: false });
        } else {
          throw new Error("Insufficient data");
        }
      })
      .catch(() => setCo2((p) => ({ ...p, isLive: false, loading: false })));
  }, []);

  // Fetch Gini from World Bank
  useEffect(() => {
    fetch("https://api.worldbank.org/v2/country/AUS/indicator/SI.POV.GINI?format=json&mrv=30&per_page=50")
      .then((r) => r.json())
      .then((json) => {
        const livePoints = (json[1] || [])
          .filter((d) => d.value !== null && parseInt(d.date) >= 2004)
          .map((d) => ({ year: parseInt(d.date), value: parseFloat(d.value) }))
          .sort((a, b) => a.year - b.year);

        if (livePoints.length >= 2) {
          // Merge live WB points over embedded interpolated data
          const merged = GINI_FALLBACK.map((e) => {
            const live = livePoints.find((r) => r.year === e.year);
            return live ? { ...live, isLive: true } : e;
          });
          setGini({ data: merged, isLive: true, loading: false });
        } else {
          throw new Error("Too sparse");
        }
      })
      .catch(() => setGini((p) => ({ ...p, isLive: false, loading: false })));
  }, []);
    ...
    <div style={{
      ...
      <div style={{ maxWidth: "740px", margin: "0 auto" }}>
        ...
        <Panel
          ...
          note="World Bank survey data merged with ABS interpolation for inter-survey years. Lower = more equal. Peak ~2008 during mining boom."
          data={gini.data}
          dataKey="value"
          unit=""
          color="#fbbf24"
          gradId="gradGini"
          domain={[30, 36]}
          yticks={[30, 31, 32, 33, 34, 35, 36]}
          source="World Bank SI.POV.GINI + ABS Cat. 6523.0"
          isLive={gini.isLive}
          loading={gini.loading}
          lastValue={lastGini}
          firstValue={gini.data[0]?.value}
        />
        ...
        <Panel
          ...
          data={WEALTH_GINI_DATA}
          dataKey="value"
          unit=""
          color="#f472b6"
          gradId="gradWealthGini"
          domain={[55, 72]}
          yticks={[55, 58, 61, 64, 67, 70]}
          refLine={{ y: 59.8, label: "2004 baseline 59.8" }}
          source="Credit Suisse/UBS Global Wealth Report · ABS/HILDA (pre-2010)"
          isLive={false}
          loading={false}
          lastValue={WEALTH_GINI_DATA[WEALTH_GINI_DATA.length - 1]?.value}
          firstValue={WEALTH_GINI_DATA[0]?.value}
        />
        ...
        <Panel
          ...
          note="Embedded ABS data. No public REST API available. COVID-era zero-rate surge (2021) drove ratio to near-record 9.8× in 2022. A ratio of 3× is considered 'affordable' by Demographia."
          data={HOUSING_DATA}
          dataKey="ratio"
          unit="×"
          color="#a78bfa"
          ...
          isLive={false}
          loading={false}
          lastValue={lastHousing}
          firstValue={HOUSING_DATA[0]?.ratio}
        />
        ...
        <div style={{ textAlign: "center", marginTop: "8px" }}>
          <p style={{
            fontFamily: "'JetBrains Mono', monospace",
            fontSize: "10px", color: "#131e30",
            letterSpacing: "0.05em",
          }}>
            LIVE sources polled on load · Embedded data from authoritative public sources · Built {new Date().toLocaleDateString("en-AU", { month: "short", year: "numeric" })}
          </p>
        </div>
      </div>
    </div>
